...
static void window_appear(Window *window) {
  ...
  const GEdgeInsets finish_insets = {
    .top = DIALOG_MESSAGE_WINDOW_MARGIN + bitmap_bounds.size.h + PBL_IF_ROUND_ELSE(5, -2),
    .right = PBL_IF_ROUND_ELSE(DIALOG_MESSAGE_WINDOW_MARGIN, DIALOG_MESSAGE_WINDOW_MARGIN / 2),
    .left = PBL_IF_ROUND_ELSE(DIALOG_MESSAGE_WINDOW_MARGIN, DIALOG_MESSAGE_WINDOW_MARGIN / 2)};
...
void alert_window_push() {
  if(!s_main_window) {
    APP_LOG(APP_LOG_LEVEL_INFO, "Creating alert window");
